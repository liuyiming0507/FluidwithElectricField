...
volt_left = "on_boundary && near(x[0], bd_left) && x[1]<=5 &&x[1]>=2"
volt_right = "on_boundary && near(x[0], bd_right) && x[1]<=5 &&x[1]>=2"
...
voltleft = CompiledSubDomain(volt_left, bd_left=bd_left)
voltleft.mark(sub_domains,6)
voltright = CompiledSubDomain(volt_right, bd_right=bd_right)
voltright.mark(sub_domains,7)
...
n=0.599 
g_ref = 9.8E3   #mm/s2
sig_ref =31.92   #muN/mm
rho_ref =0.89E-3  #g/mm3
x_ref = 4.    #mm
u_ref = 100#(x_ref*g_ref)**0.5 #mm/s
shear_rate_ref = (u_ref/x_ref)**(n-1) #/s
K_ref=1000.        #Pa s2
mu_ref= 0.039 #K_ref*shear_rate_ref
...
bc.append(DirichletBC(Space.sub(1), Constant(Voltage/2), voltleft))
bc.append(DirichletBC(Space.sub(1), Constant(-Voltage/2), voltright))
bc.append(DirichletBC(Space.sub(2), Constant((0.,0.)), bdleft))
bc.append(DirichletBC(Space.sub(2), Constant((0.,0.)), bdright))
bc.append(DirichletBC(Space.sub(2), Constant((0.,0.)), voltleft))
bc.append(DirichletBC(Space.sub(2), Constant((0.,0.)), voltright))
bc.append(DirichletBC(Space.sub(2), Constant((0.,-Amp)), bdtop))
bc.append(DirichletBC(Space.sub(2), Constant((0.,-Amp)), bdbottom))
...
file_p = XDMFFile(foldername+'/p_Re%.6s' %Re + '_dt%.5s'%dt + '_res%d'%res + '_We%.3s'%We+ '_El%.3s'%El+ '_Fr%.3s.xdmf'%Fr)
file_Voltage = XDMFFile(foldername+'/Voltage_Re%.6s' %Re + '_dt%.5s'%dt + '_res%d'%res + '_We%.3s'%We+ '_El%.3s'%El+ '_Fr%.3s.xdmf'%Fr)
file_v = XDMFFile(foldername+'/v_Re%.6s' %Re + '_dt%.5s'%dt + '_res%d'%res + '_We%.3s'%We+ '_El%.3s'%El+ '_Fr%.3s.xdmf'%Fr)
file_phi = XDMFFile(foldername+'/phi_Re%.6s' %Re + '_dt%.5s'%dt + '_res%d'%res + '_We%.3s'%We+ '_El%.3s'%El+ '_Fr%.3s.xdmf'%Fr)
file_E = XDMFFile(foldername+'/E_Re%.6s' %Re + '_dt%.5s'%dt + '_res%d'%res + '_We%.3s'%We+ '_El%.3s'%El+ '_Fr%.3s.xdmf'%Fr)
file_fs = XDMFFile(foldername+'/fs_Re%d' %Re + '_dt%.5s'%dt + '_res%d'%res + '_We%.3s'%We+ '_El%.5s'%El+'_Fr%.3s.xdmf'%Fr)
file_fe = XDMFFile(foldername+'/fe_Re%d' %Re + '_dt%.5s'%dt + '_res%d'%res + '_We%.3s'%We+ '_El%.5s'%El+'_Fr%.3s.xdmf'%Fr)
file_ratio = XDMFFile(foldername+'/ratio_Re%d' %Re + '_dt%.5s'%dt + '_res%d'%res + '_We%.3s'%We+ '_El%.5s'%El+'_Fr%.3s.xdmf'%Fr)
file_Ts = XDMFFile(foldername+'/Ts_Re%d' %Re + '_dt%.5s'%dt + '_res%d'%res + '_We%.3s'%We+ '_El%.5s'%El+'_Fr%.3s.xdmf'%Fr)
file_Te = XDMFFile(foldername+'/Te_Re%d' %Re + '_dt%.5s'%dt + '_res%d'%res + '_We%.3s'%We+ '_El%.5s'%El+'_Fr%.3s.xdmf'%Fr)
...
file_ratio.write_checkpoint(ratio, "ratio", t, append=False)
...
while t < t_end:
	...
	assign(phi_c0,phi_c)
	E.assign(project(E_,VectorSpaceE))
	if nn == 1 or (nn % 100 == 0 and nn<=10000000):
		...
		file_ratio.write_checkpoint(ratio, "ratio", t, append=True)
