...
# -*- coding: utf-8 -*-
"""
Created on Jun 16 22:13:38 2023
...
res=15 ##resolution: set the number following by the fname which depends on the selected mesh
scale=1
t_end = 5#8
B=0.015  #0.001*res
dt = B*(scale/res)
...
fname = "geofolder/Jetflow_EM_thinres15"#resolution is 15
mesh = Mesh(fname + '.xml')
...
bc.append(DirichletBC(Space.sub(1), Constant((0.,0.)), noslip))
bc.append(DirichletBC(Space.sub(1), Constant((0.,-Amp)), piptop))
...
file_fs = XDMFFile(foldername+'/fs_Re%.6s' %Re + '_dt%.5s'%dt + '_res%d'%res + '_We%.3s'%We+ '_El%.5s'%El+'_Fr%.3s.xdmf'%Fr)
#file_fe = XDMFFile(foldername+'/fe_Re%d' %Re + '_dt%.5s'%dt + '_res%d'%res + '_We%.3s'%We+ '_El%.5s'%El+'_Fr%.3s.xdmf'%Fr)
file_Ts = XDMFFile(foldername+'/Ts_Re%.6s' %Re + '_dt%.5s'%dt + '_res%d'%res + '_We%.3s'%We+ '_El%.5s'%El+'_Fr%.3s.xdmf'%Fr)
...
file_Ts.write_checkpoint(Ts, "Ts", t, append=False)
...
Is = Identity(len(n_phi)) - outer(n_phi, n_phi)
dlta = pow(dot( grad(phi_c), grad(phi_c) ), 0.5)
T_s = (Is + mu_ref2/sig_ref*dot(Is,d_)*Is)*dlta
#weak forms
F_momentum = (rou*dot((u-u0),v)/dt + rou*c(u,u,v) + (1. / Re) * inner(tau,grad(v)) + e(p, v) - (1./(Fr*Fr))*rou*dot(g,v) + (1./We)*inner(T_s,grad(v)) ) * dv 

F_mass = b(u,q)*dv
#F_affixation = (dot((u-u0),grad(q)) - (dt/(Fr*Fr))*dot(g,grad(q)) + dt*c(u,u,grad(q)) + (dt/rou)*e(p, grad(q)) - (dt/rou)*(1./Re)*dot(div(tau),grad(q)) ) * dv
F_affixation = (dot((u-u0),grad(q)) - (dt/(Fr*Fr))*dot(g,grad(q)) + dt*c(u,u,grad(q)) + (dt/rou)*e(p, grad(q)) - (dt/rou)*(1./Re)*dot(div(tau),grad(q))- (dt/rou)*(1./We)*dot(div(T_s),grad(q)) ) * dv
...
while t < t_end:
	...
	if nn == 1 or (nn % 100 == 0 and nn<=10000000):
		...
		file_Ts.write_checkpoint(Ts, "Ts", t, append=True)
